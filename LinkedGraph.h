...
public:
   ...
   */
   bool LinkedGraph<LabelType>::saveToFile(LabelType startLabel, ofstream &ofs);
...
bool LinkedGraph<LabelType>::saveToFile(LabelType startLabel, ofstream &ofs)
{
	...
	Vertex<LabelType>* startVertex = vertices.getItem(startLabel);
	queue<Vertex<LabelType>*> tempQueue;
	tempQueue.push(startVertex);
	...
	{
		theVertex = tempQueue.front();
		theVertex->visit();
		theVertex->resetNeighbor();
		LabelType startVertex = theVertex->getLabel();

		for (int n = 0; n < theVertex->getNumberOfNeighbors(); n++)
		{
			LabelType endVertex = theVertex->getNextNeighbor();
			if (!(vertices.getItem(endVertex))->isVisited())
			{
				int weight = theVertex->getEdgeWeight(endVertex);
				ofs << startVertex << endl << endVertex << endl << weight << endl;
			}
